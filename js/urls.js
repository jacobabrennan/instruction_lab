// Achtung! There are urls that need to be edited at the top of the file navvid_mozilla.css
var urls = {
    logo1: 'images/ignite_logo.svg',
    logo2: 'images/sdn_logo.svg',
    time_stamp_play: 'images/play_button.svg',
    ignite: 'images/ignite_embossed_logo.svg',
    ordinals: 'images/ignite_embossed_logo.svg',
    video: {
        intro: {
            'mp4': 'video/sdn_intro.mp4',
            'webm': 'video/sdn_intro.webm',
            'ogv': 'video/sdn_intro.ogv'
        },
        lab_setup: {
            'mp4': 'video/sdn_diy_setup.mp4',
            'webm': 'video/sdn_diy_setup.webm',
            'ogv': 'video/sdn_diy_setup.ogv'
        },
        lab_1: {
            'mp4': 'video/sdn_diy_01.mp4',
            'webm': 'video/sdn_diy_01.webm',
            'ogv': 'video/sdn_diy_01.ogv'
        },
        lab_2: {
            'mp4': 'video/sdn_diy_02.mp4',
            'webm': 'video/sdn_diy_02.webm',
            'ogv': 'video/sdn_diy_02.ogv'
        },
        lab_3: {
            'mp4': 'video/sdn_diy_03.mp4',
            'webm': 'video/sdn_diy_03.webm',
            'ogv': 'video/sdn_diy_03.ogv'
        },
        deep_dive: {
            'mp4': 'video/sdn_deepdive.mp4',
            'webm': 'video/sdn_deepdive.webm',
            'ogv': 'video/sdn_deepdive.ogv'
        }
    },
    tip_icons: {
        link_out: 'images/linkbox_padding.svg',
        github: 'images/octocatvector.svg',
        code: 'images/utilities-terminal.svg',
        youtube: 'images/youtube_logo.svg',
        home: 'images/home.svg',
        router: 'images/router.svg',
        bigswitch: 'images/bigswitch.png',
        floodlight: 'images/floodlight.png',
        geni: 'images/geni.png',
        marist: 'images/marist.png',
        onf: 'images/onf.png',
        person: 'images/person.png',
        pox: 'images/pox.png',
        raspberrypi: 'images/raspberrypi.png',
        stanford: 'images/stanford.png',
        wikipedia: 'images/wikipedia.png',
        email: 'images/mail-forward.svg',
        openflow: 'images/openflow.png'
    }
};